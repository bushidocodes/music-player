...
import type { AppAction, Playlist, Song } from '../types';
import { apiFetch, convertSong } from '../utils';
...
export const getPlaylistById =
  (playlistId: string): AppThunk<Promise<void>> =>
  async (dispatch) => {
    try {
...
export const addNewPlaylist =
  (playlistName: string, navigate: NavigateFunction): AppThunk<Promise<void>> =>
  async (dispatch, getState) => {
    try {
      const playlist = await apiFetch<Playlist>('/api/playlists', {
        method: 'POST',
...
export const addSongToPlaylist =
  ...
  async (dispatch, getState) => {
    const song = await apiFetch<Song>(`/api/playlists/${playlistId}/songs`, {
      method: 'POST',
      body: { id: songId },
    });
